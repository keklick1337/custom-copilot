...
async function performCommitMsgGeneration(secrets: vscode.SecretStorage, gitDiff: string, inputBox: any) {
	...
	try {
		vscode.commands.executeCommand("setContext", "customcopilot.isGeneratingCommit", true);
		const config = vscode.workspace.getConfiguration();

		// Get custom prompts or use defaults
		const customSystemPrompt = config.get<string>("customcopilot.commitMessagePrompt", "");
		const PROMPT = {
		...
		const baseUrl = selectedModel.baseUrl || config.get<string>("customcopilot.baseUrl", "");
		...
		vscode.commands.executeCommand("setContext", "customcopilot.isGeneratingCommit", false);
	}
}
...
async function ensureApiKey(secrets: vscode.SecretStorage, provider: string): Promise<string | undefined> {
	...
	return secrets.get(`customcopilot.apiKey.${normalizedProvider}`);
}
